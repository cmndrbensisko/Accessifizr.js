...
function (declare, lang, topic, dom, query, domAttr, domConstruct, JSON, on, keys, array, domClass, Deferred, focusUtil) {               
	...
	init: function(args){
			...
			lang.mixin(options,args);
			handlerBus = []
			parseData = JSON.parse(options.data);
			setParent(parseData.tabOrder, null)
				...
				query(createdObject.id).forEach(function(node){
					...
					if (lang.exists('onClick', createdObject)){
						...
						if (lang.exists('objects', createdObject.onClick)){
							tabBus.push(function(){assignTabs(createdObject.onClick)})
						}
						if (lang.exists('back', createdObject.onClick)){
							tabBus.push(function(){assignTabs(recurseParent(getObjects(parseData,'id',createdObject.id)[0],createdObject.onClick.back))})
						}
						handlerBus.push(on(node,getEventString(createdObject,"clickModes"),lang.hitch(this,function(evt){
							if (checkEvt(evt,createdObject,"clickModes")){
								...
								if (evt.type !== emitType){
									on.emit(emitNode,emitType,{"bubbles":true})
								}else{
									tabBus.forEach(function(callback){callback();});
					...
					if (lang.exists('onEsc',createdObject)){
						handlerBus.push(on(node,getEventString(createdObject, "escModes"), function(evt){
							if (checkEvt(evt,createdObject,"escModes")){
								...
								}else{emitNode = query(emitNode)[0]};
								if (evt.type !== createdObject.onEsc.escControlEvt){
									on.emit(emitNode,createdObject.onEsc.escControlEvt,{"bubbles":true})
								}
							}
						}))
					};
	...
	function clearTabs(){
		...
		//assume this is because the controls are out of mode; clear the bus
	...
	function getSwapType(swapType,target){
		var swapTypes = {
			'div': function(target){
				...
				on(newNode, "click",function(e){on.emit(target,'click',{})})
				on(newNode, "keydown",function(e){on.emit(target,'keydown',{})})
				if (target.id == ''){
					target.id = makeId()
				}
	...
	function setTabs(node, index){
		...
		if (options.applyAria){watchResult(node)};
	}
	...
	function watchResult(target){
		if (array.indexOf(options.exemptTags,target.nodeName) == -1){
			if (domAttr.get(target, "tabindex") == 0||domAttr.get(target, "tabindex") > 0){
				domAttr.set(target, "aria-hidden", "false")
				if (checkForSwap(target)){swapForAccess('img',target)}
				domAttr.remove(target, "role");
				var el = target
				//Climb up; If this is visible, need to make anything above visible.
				while (null !== findUpTag(el)){
					domAttr.set(el, "aria-hidden", "false")
					//Are you ever going to have a parent image, really?
					//if (swapImgDiv){swapForAccess('img',el)};
					domAttr.remove(el, "role");
					el = findUpTag(el);
				}
				//Crawl down; Any children are assumed visible unless otherwise set after the fact
				dojo.query("*", target).forEach(function(node){
					if (!(domClass.contains(node,"alwaysHidden"))){domAttr.set(node, "aria-hidden", "false")};
					if (checkForSwap(node)){swapForAccess('img',node)};
				})
			}
			if (domAttr.get(target, "tabindex") == -1){
				domAttr.set(target, "aria-hidden", "true")
				if (checkForSwap(target)){swapForAccess('div',target)};
				domAttr.set(target, "role", "presentation")
				//if it has a parent header, hide it too
				//really you should just hide the top wrapper you want hidden, and let crawldown take care of the rest. this code is unnecessary
				if (!(target.parentNode == null)){
					if (target.parentNode.nodeName == "H2"){
						domAttr.set(target.parentNode, "aria-hidden", "true")
						if (checkForSwap(target.parentNode)){swapForAccess('div',target.parentNode)};
						domAttr.set(target.parentNode, "role", "presentation")
					}
				}
				//Crawldown; hide all children.  Also, if all siblings are hidden too, it should hide the parent (and so on).
				dojo.query("*", target).forEach(function(node){
					if (domAttr.get(node, "tabindex") == 0||domAttr.get(node, "tabindex") > 0){
						...
						while (null !== findUpTag(el)){
							domAttr.set(el, "aria-hidden", "false")
							if (checkForSwap(el)){swapForAccess('img',el)}
							domAttr.remove(el, "role")
							el = findUpTag(el);
						}
					}
